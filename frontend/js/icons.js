/**
 * NutriFit — Sistema centralizado de iconos SVG (reemplazo de emojis de
 * interfaz, ver "Personalización 2.0 + sistema profesional de iconos").
 * =====================================================================
 * Vendorizados localmente desde Lucide Icons (lucide-static, licencia
 * ISC) — se descargó una sola vez el SVG de cada icono usado y se guardó
 * acá el contenido interno (paths/circles), sin depender de ningún CDN ni
 * librería en tiempo de ejecución. Todos comparten el mismo estilo:
 * stroke=currentColor, stroke-width 2, 24×24 viewBox — así se ven
 * consistentes entre sí y heredan el color de texto/tema automáticamente.
 *
 * Uso: Icon('flame') devuelve el <svg> como string HTML, listo para
 * insertar en un template. Es un <script> clásico (no module) a propósito
 * — se carga antes que app.js/auth.js y expone `Icon`/`ICONS` en window,
 * igual que Api/loadView/etc.
 */
const ICON_PATHS = {
  ...
  // Agregados para la revisión de Onboarding + Perfil:
  ...
  nut: '<path d="M12 4V2" /><path d="M5 10v4a7.004 7.004 0 0 0 5.277 6.787c.412.104.802.292 1.102.592L12 22l.621-.621c.3-.3.69-.488 1.102-.592A7.003 7.003 0 0 0 19 14v-4" /><path d="M12 4C8 4 4.5 6 4.5 8c-.243.97-.919 1.952-2 3 1.31-.082 1.972-.29 3-1 .54.92.982 1.356 2 2 1.452-.647 1.954-1.098 2.5-2 .595.995 1.151 1.427 2.5 2 1.31-.621 1.862-1.058 2.5-2 .629.977 1.162 1.423 2.5 2 1.209-.548 1.68-.967 2-2 1.032.916 1.683 1.157 3 1-1.297-1.036-1.758-2.03-2-3-.5-2-4-4-8-4Z" />',
...
/**
 * Icon(name, opts) → string HTML de un <svg> listo para insertar.
 * opts.size (default 20), opts.strokeWidth (default 2), opts.className,
 * opts.label (si se pasa, el icono deja de ser decorativo y se anuncia
 * con role="img" + aria-label — ver punto 21, accesibilidad).
 */
function Icon(name, opts = {}) {
  ...
  return `<svg class="icon-svg${className ? ` ${className}` : ''}" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${strokeWidth}" stroke-linecap="round" stroke-linejoin="round" ${a11y}>${body}</svg>`;
}

/**
 * NutriCoinIcon(size) — único punto de la app que dibuja el ícono de
 * NutriCoin (punto 16: "preparar el sistema para que la moneda se
 * pueda reemplazar fácilmente por nutricoin.svg/webp sin cambiar la
 * lógica"). Hoy usa el SVG "coins" de Lucide en verde-oro de marca; el
 * día que exista el asset 3D/ilustrado propio, éste es el único lugar
 * que hay que tocar — cada llamador (Home, tienda, ranking, user-chip)
 * ya usa esta función en vez de un emoji suelto.
 */
function NutriCoinIcon(size = 16) {
  return `<svg class="icon-svg icon-coin" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${ICON_PATHS.coins}</svg>`;
}

/** Hidrata cualquier `<span data-icon="nombre">` presente en el HTML
 *  estático (nav sidebar/bottom nav, botones fijos) con su SVG real. El
 *  contenido generado dinámicamente por app.js llama a Icon(...) directo
 *  en el template, no necesita pasar por acá. */
...
if (typeof window !== 'undefined') {
  ...
  window.NutriCoinIcon = NutriCoinIcon;
  hydrateIcons();
}
